...
uint64 shift_value(uint64 val, ShiftType type, int amount, bool sf) {
    ...
    case SHIFT_ROR:
        ...
        if (sf)
            ...
            val = (val >> amount) |
                  ((val & BITMASK_LOWER_32_BITS) << (32 - amount));
        break;
    }
...
// Shared helper function for register and immediate data processing
// instructions
void process_arithmetic(State *state, Register rd, uint64 val_n, uint64 op2,
                        bool is_sub, bool set_flags, bool sf) {
    // Maths
    ...
    if (set_flags) {
        ...
        if (is_sub) {
            ...
            v_flag = (((val_n ^ op2) & sign_bit) &&
                      ((val_n ^ result) & sign_bit)) != 0;
...
void execute_logical_register(State *state, Instruction *i) {
    ...
    // Negating
    if (op == OP_TYPE_BIC || op == OP_TYPE_BICS || op == OP_TYPE_ORN ||
        op == OP_TYPE_EON) {
        op2 = ~op2;
    }
...
void execute_wide_move(State *state, Instruction *i) {
    ...
    case OP_TYPE_MOVK: {
        ...
        uint64 existing_value =
            sf ? read_reg_64(state, rd) : (uint64)read_reg_32(state, rd);
...
uint64 find_address(State *state, SingleDataTransferInstruction data_transfer) {
    ...
    case ADDR_UNSIGNED_OFFSET: {
        uint64 uoffset = data_transfer.sf ? (uint64)data_transfer.offset * 8
                                          : (uint64)data_transfer.offset * 4;
        return xn + uoffset;
    }
...
void execute_single_data_transfer(State *state, Instruction *i) {
    ...
    if (data_transfer.mode == ADDR_PRE_INDEXED ||
        data_transfer.mode == ADDR_POST_INDEXED) {
